...
DWORD PrintPushEvent(EVT_HANDLE hEvent);


void GetPushEvent(LPCWSTR pwsPath, LPCWSTR pwsQuery)
{
	...
	aWaitHandle = GetStdHandle(STD_INPUT_HANDLE);
	if (INVALID_HANDLE_VALUE == aWaitHandle)
	{
...
DWORD WINAPI SubscriptionCallback(EVT_SUBSCRIBE_NOTIFY_ACTION action, PVOID pContext, EVT_HANDLE hEvent)
{
	...
	case EvtSubscribeActionDeliver:
		if (ERROR_SUCCESS != (status = PrintPushEvent(hEvent)))
		{
			goto cleanup;
		}
		break;
...
// Render the push event as an XML string and print it.
DWORD PrintPushEvent(EVT_HANDLE hEvent)
{
	DWORD status = ERROR_SUCCESS;
